...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "--host 203.0.113.10 --root-user root --ssh-key /root/.ssh/id_ed25519 "
            "--user ipwall --group ipwall "
    ...
    parser.add_argument(
        "--skip-ssh-check",
        action="store_true",
        help="Skip initial SSH connectivity/authentication preflight check",
    )
...
def run_command(cmd, dry_run=False):
    ...
    try:
        ...
        raise RuntimeError(
            "command failed "
            f"(exit={exc.returncode}): {quote(exc.cmd)}",
        ) from exc


def preflight_ssh(args):
    run_command(ssh_base(args) + ["true"], dry_run=False)
...
def main():
    ...
    if not args.skip_ssh_check and not args.dry_run:
        try:
            preflight_ssh(args)
        except RuntimeError as exc:
            print("ERROR: Unable to connect/authenticate to the remote host over SSH.", file=sys.stderr)
            print(f"DETAIL: {exc}", file=sys.stderr)
            print("HINT: Use --ssh-key for the root login key used to reach the remote host.", file=sys.stderr)
            print(
                "HINT: --authorized-key-file is for the remote ipwall user's authorized_keys and should be a PUBLIC key.",
                file=sys.stderr,
            )
            return 1

    validated_key_lines = None
    if args.authorized_key_file:
        ...
        if not key_lines:
            ...
            return 1
        if any("PRIVATE KEY" in line for line in key_lines):
            print(
                f"ERROR: authorized key file appears to be a private key: {key_path}. "
                "Use a public key (.pub) for --authorized-key-file.",
                file=sys.stderr,
            )
            return 1
        validated_key_lines = key_lines
    ...
    if args.authorized_key_file:
        ...
        restricted_lines = []
        for key_line in (validated_key_lines or []):
